...
bootstrap:
	...
	# 3. Apply the Prometheus CRDs imperatively. This is necessary before deploying the monitoring stack.
	@just install-prometheus-crds
	@echo "--> Applying Argo CD manifests..."
	# 4. Apply the rendered Argo CD manifests to install or upgrade Argo CD.
	...
	# 5. Grant the Argo CD controller cluster-admin rights to manage applications.
	@kubectl apply -f bootstrap/argocd-controller-clusterrolebinding.yaml
	# 6. Apply all bootstrap resources
